...
class HomeScene(Scene):
    ...
    def __init__(self, mainApp: pygame.Surface,
        ...
        self.title_image = pygame.image.load(
            os.path.join(images_dir, "Title.png")).convert_alpha()
        ...
        self.title_rect = self.title_image.get_rect(
            centerx=settings.WINDOW_SIZE[0] // 2, top=-20)
        ...
        btn_images_raw = [
            pygame.image.load(os.path.join(images_dir,
                                           "Solo.png")).convert_alpha(),
            pygame.transform.scale(
                pygame.image.load(os.path.join(
                    images_dir, "Face-a-Face.png")).convert_alpha(),
                (400, 200)),
            pygame.transform.scale(
                pygame.image.load(
                    os.path.join(images_dir,
                                 "Record mondial.png")).convert_alpha(),
                (400, 200))
        ]
    ...
    def _preload_cached_images(self):
        ...
        num_samples = settings.FRAMERATE
        ...
        for i in range(num_samples):
            ...
            scaled_img = pygame.transform.smoothscale(self._title_base_image,
                                                      (scaled_w, scaled_h))

            vertical_bump = int(
                (hb_scale - self._heart_scale_min) /
        ...
        for i in range(glow_samples):
            ...
            for layer in range(2):
                ...
                pygame.draw.rect(glow_surface, (*border_color[:3], alpha),
                                 glow_rect, border_thickness - layer)
    ...
    def loopScene(self, events: List[pygame.event.Event]):
        ...
        self.title_image, vertical_bump = self._title_cache[
            self._title_cache_index]
        self.title_rect = self.title_image.get_rect(
            centerx=settings.WINDOW_SIZE[0] // 2, top=-20 - vertical_bump)
        ...
        for i, img in enumerate(self.btn_images):
            ...
            if is_selected:
                ...
                glow_surface, tint_overlay, glow_padding = self._glow_cache[
                    self._glow_cache_index]
